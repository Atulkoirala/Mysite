...
export default function Skill(props) {
  ...
  useEffect(() => {
    ...
    const getload = async () => {
      try {
        const response = await fetch('http://127.0.0.1:8000/media/skill');
      ...
      {post.map((elem) => {
              return (
                ...
                <Grid key={elem.id} item sx={{ position: 'flex' }}>
                  <Card component={motion.button} onClick={()=>(modalOpens ? close(elem.Title,elem.Description,elem.Svg_Image,elem.Image,elem.id) : open(elem.Title,elem.Description,elem.Svg_Image,elem.Image,elem.id))} sx={{ width: 100, height: 80, borderRadius: '2rem', backgroundColor: 'black', position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <Typography component={motion.p} p={1} sx={{ fontSize: '1rem',margin:'auto', textAlign: 'center', fontWeight: 'bolder',color:'white',padding:'1rem 1rem', justifyContent: 'center', alignItems: 'center' }}>{elem.Title} </Typography>
                  <Card component={motion.button} onClick={()=>(modalOpens ? close(elem.Title,elem.Description,elem.Svg_Image,elem.Image,elem.id) : open(elem.Title,elem.Description,elem.Svg_Image,elem.Image,elem.id))}  whileHover={{ opacity: '0' }} sx={{ position: 'absolute',borderRadius: '2rem',border:'1.5px solid black', backgroundColor: 'white', top: 0, left: 0, bottom: 0, right: 0, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
