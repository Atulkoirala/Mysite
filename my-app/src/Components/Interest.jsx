...
export default function Interest() {
  ...
  const getload = async()=>{
      const response = await axios.get('http://127.0.0.1:8000/media/intrest');
      ...
      <Box 
          ...
          p={2} sx={{borderRadius: '5rem 5rem 5rem 5rem',margin:'auto'}}
              ...
              <Grid container spacing={5}>
                {post.map((elem) => {
                  return (
                    <Grid key={elem.id} item sx={{ position: 'flex',marginX:{xl:'1.8rem',lg:'1.5rem',md:'1.2rem',sm:'0.7rem',xs:'0.1rem'}}}>
